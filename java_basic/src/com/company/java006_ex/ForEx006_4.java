	...
	public static void main(String[] args) {
		...
		/* break ( 빠져나와!) / continue (밑에꺼 skip)
		 for(;;){  // 1-1. 무한반복
		 	...
		 	// #todo else if(잘못썼으면){ 숫자2입력받기 } 
		 	// #todo else if(잘못썼으면){ 연산자입력받기 }
		 	// 1-2. else{빠져나올 조건-빠져나오기 break}
		 }
		 */
		for(;;) {
			if(!(a>=0 && a<=100)) { // 잘쓴범위가 아니라면 입력시도 / 잘쓴범위면 입력 X
				System.out.print("정수를 하나 입력해주세요 > "); a = scanner.nextInt();
			}
			else if(b<0 || b>100) { // -1  ||  101  
				System.out.print("정수를 하나 입력해주세요 > "); b = scanner.nextInt();
			}
			else if(!(ch=='+' || ch=='-' || ch=='*' || ch=='/')) {
				System.out.print("연산자를 입력해주세요(+,-,*,/) > "); ch = scanner.next().charAt(0);
			}else {break;}
		}
