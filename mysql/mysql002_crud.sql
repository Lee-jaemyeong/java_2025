...
#[004]
alter table score modify ssavg double not null; # 자료형 double로 바꾸기
...
select *from userinfo_delete;    # ctrl + enter 실행
...
desc emp_del;
alter table emp_del modify no int not null auto_increment primary key;
select *from emp_del;

# [001] [TABLE : emp_del] (직책(JOB)이  'SALESMAN'인 데이터를 삭제하시오.  ) 
delete from emp_del where job='SALESMAN';
# [002] [TABLE : emp_del] (직책(JOB)이  'MANAGER'이고  이름(ENAME)이 'JONES'인 데이터를 삭제하시오.  )
delete from emp_del where job='MANAGER' and name='JONES';
# [003] [TABLE : emp_del] (delete_emp 테이블의 모든 데이터를 삭제하시오. ) 
delete from emp_del;
