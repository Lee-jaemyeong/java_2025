package com.thejoa.project001.util;
...
public class PagingDto {
   ...
   private int onepagelist; // #2. 한 페이지에 보여줄 게시물의 10
   private int pagetotal;   // #3. 총 페이지 수는 123/10 = 13 12페이지 + 3글
   private int bottomlist;  // #4. 하단의 페이지 나누기  이전 11 12
   
   private int pstartno;    // #5. 페이지의 스타트번호 1page : 최신글 0~9 
   
   private int current;     // #6. 현재 페이지번호
   private int start;       // #7. 시작 페이지번호
   private int end;         // #8. 끝 페이지번호
   ...
   public PagingDto(int listtotal, int pstartno) {   
      ...
      // 1(0)	2(10)	3(20)
      // 현재 페이지 pstartno=10이라면 현재페이지번호는 2
      // 현재 페이지 pstartno=20이라면 현재페이지번호는 3
      
      this.start = ((this.current-1)/this.bottomlist) * this.bottomlist+1; 
      //  11	15(현재)	20
      //  11을 11로, 15를 11로, 20을 11로
      //  11-1=10/10	15-1=14/10	  20-1=19/10 > 앞자리 1 > 1*10+1
